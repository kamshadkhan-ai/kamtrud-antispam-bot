Шаг 1: Создай файлы на своём компьютере (2 минуты)

Открой любой текстовый редактор (Блокнот в Windows, TextEdit в Mac, или VS Code, если есть).
Создай файл main.py и вставь в него код ниже.
Создай файл requirements.txt и вставь в него:
aiogram==3.4.1
googletrans==4.0.0rc1
langdetect==1.0.9
Создай файл README.md (опционально) с этой инструкцией, если хочешь.
Полный код бота (main.py)
Скопируй ровно это (твой токен уже вставлен, все функции из диалога включены):
import asyncio
import logging
...
async def mark_ham(message: Message):
    if message.reply_to_message:
        ...
        await message.answer("Ответь на сообщение командой /ham")@dp.message(Command("welcome"))
...
Шаг 2: Установка и запуск (5–10 минут)

Установи Python (если нет): Скачай с https://python.org/downloads (версия 3.10+). При установке поставь галочку "Add to PATH".
Открой командную строку (Win+R → cmd).
Перейди в папку с файлами: cd путь_к_папке (например, cd Desktop\bot).
Установи библиотеки: pip install -r requirements.txt
Запусти бота: python main.py
Увидишь "Бот запущен!" — значит, работает.
Добавь бота в группу: https://t.me/kam_trud_guard_bot?startgroup=true (или найди по username, который ты выбрал в BotFather).
Дай права админа: Удалять сообщения + Блокировать пользователей.

Шаг 3: Развёртывание 24/7 (бесплатно, 5 минут)

Replit (самый простой):
Иди на https://replit.com/signup (регистрируйся).
Создай новый Repl: "Python".
Вставь код в main.py, requirements в requirements.txt.
Нажми Run — бот запустится. Для 24/7: в Shell набери uptime (или используй Always On в премиум, бесплатно до 1 часа/день).
Render (бесплатно навсегда):
https://render.com — зарегистрируйся.
"New → Web Service" → "Build and deploy from a Git repository" (создай репозиторий на GitHub сам: https://github.com/new, загрузи файлы туда).
Укажи репозиторий, Runtime: Python, Build: pip install -r requirements.txt, Start: python main.py.
Deploy — через 2 минуты бот онлайн.
Heroku или Railway: Аналогично, инструкции на их сайтах.

Тестирование:

Напиши в группу: "Купить права через ГИБДД" — должно удалить + забанить.
На английском: "Quick money trading" — переведёт и удалит.
Для обучения: Ответь на сообщение /spam или /ham.
Если ошибка при запуске (например, с langdetect) — пиши сюда текст ошибки, поправлю код за секунду. Или пришли скрин — разберёмся.
Теперь это реально твой бот — запускай и пользуйся! Если нужно доработки (добавить капчу, БД для хранения), скажи. 😊